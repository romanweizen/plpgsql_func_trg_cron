...
BEGIN
	...
	Коротко: <>/!= ломаются на NULL, а IS DISTINCT FROM — нет. Поэтому в триггерах и сравнениях «изменилось/не изменилось» правильно использовать именно IS DISTINCT FROM.
 	Почему так
	SQL живёт в трёхзначной логике: TRUE, FALSE, UNKNOWN. Любая обычная операция сравнения с участием NULL даёт UNKNOWN, а в WHERE это значит «строку не берём».
	a <> b и a != b эквивалентны и стандартны для «не равно».
	Смысл: IS DISTINCT FROM — это «безопасное для NULL сравнение на неравенство». Пара NULL и значение считаются различными, а два NULL считаются неразличимыми.
	Но если хотя бы одно из a или b — NULL, то результат не TRUE и не FALSE, а UNKNOWN.
	В PostgreSQL для чисел с плавающей точкой NaN ведёт себя особенным образом: обычное равенство считает NaN «не равным самому себе». 
	IS DISTINCT FROM наоборот трактует «NaN не отличим от NaN», то есть два NaN считаются одинаковыми. 
	Это намеренно, чтобы сохранить «здравый смысл» при проверках изменений.
	*/
